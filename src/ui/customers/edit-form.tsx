...
import { Customer, State } from "@/lib/customers/definitions";
import { updateCustomer } from "@/lib/customers/update_action";
...
export default function EditCustomerForm({ customer }: { customer: Customer }) {
  const { name, profile_photo, email, phoneNumber, password } = customer;
  ...
  return (
    <form action={formAction}>
      ...
      >
        ...
        <div className="mb-4">
          ...
          <div className="flex gap-2 items-center">
            {(file || profile_photo) && (
              <Image
                src={file ? URL.createObjectURL(file) : profile_photo}
                alt="Preview"
                height={20}
                width={20}
      ...
      {state.success && (
        <ConfirmationModal
          text={state.message ?? "Successfully Updated The Customer"}
          // next_href="/orders"
        />
      )}
      <div className="mt-6 flex justify-end gap-4">
        <FormCancelLink href="/dashboard/customers">Cancel</FormCancelLink>
        <FormSubmitButton type="submit">Update Customer</FormSubmitButton>
      </div>
    </form>
  );
}
